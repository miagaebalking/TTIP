...
public class ProfileController {
	...
	public Profile selectAllProfileInfo(int userNo) {
		...
		int profileNo = service.selectProfileNo(userNo);
		System.out.println("profileNo 가져왔니?"+profileNo);
		int count = service.selectReviewCount(String.valueOf(profileNo));
		
		System.out.println("reviewCount:"+count);
		
		Profile profile = service.selectEveryInfoProfile(count,userNo);
		System.out.println("profile 모든 정보 제발 나와라:"+profile);
		return profile;
	}
		...
		@ResponseBody
		@GetMapping("selectReviewList.pr")
		public HashMap<String,Object> selectReviewList(@RequestParam(value="profileNo") String profileNo){
			
			System.out.println("profileNo가 무엇인가?"+profileNo);
			
			int reviewCount = service.selectReviewCount(profileNo);
			System.out.println("reviewCount:"+reviewCount);
			ArrayList<Review> reviewList = service.selectReviewList(profileNo);
			
			int reviewTotalSum = 0;
			for(Review r : reviewList) {
				
				reviewTotalSum += r.getReviewStar();
			...
			Profile profile = service.selectEveryInfoProfile(reviewCount, reviewTotalSum);
			if(reviewCount>0) {
				
				String reviewAverage = String.format("%.1f",(reviewTotalSum/(double)reviewCount));
				map = new HashMap<>();
				map.put("reviewCount", reviewCount);
				map.put("reviewAverage", reviewAverage);
				map.put("reviewList", reviewList);
				map.put("profileNo", Integer.parseInt(profileNo));
				service.updateReviewInfo(map);//리뷰 정보 프로필 테이블에 넣기
				
				
			}else {
				map = new HashMap<>();
				map.put("checkNull", false);
		...
		//------------------자격증영역-------------------------
		@ResponseBody
		@RequestMapping(value="licenseSave.pr",method = RequestMethod.POST,produces = "application/json; chatset=UTF-8")
		public ArrayList<Image> portfolioEnroll(@RequestParam MultipartFile[] saveFile
									 ,int userNo
									 ,HttpSession session) {
			...
			System.out.println(saveFile.length+"지금 요청 들어 오신거야 여기까지는");
			
			for(int i=0; i<saveFile.length; i++) {
				
				if(saveFile[i].getOriginalFilename()!="") {
					
					image = new Image();
					String changeName = saveFile(saveFile[i],session);
					
					image.setOriginName(saveFile[i].getOriginalFilename());
					image.setChangeName("resources/uploadFiles/"+changeName);
					image.setFileType("license-image");
					image.setRefNo(userNo);
					list.add(image);
			...
			int result = service.uploadLicenseImages(list);
	...
	@ResponseBody
	@RequestMapping(value="selectLicenseList.pr",produces = "application/json; chatset=UTF-8")
	public ArrayList<Image> selectLicenseList(@RequestParam int userNo){
		
		ArrayList<Image> list = new ArrayList<>();
		
		list = service.selectLicenseList(userNo);
//		System.out.println(list);
		return list;
	}
	
	@ResponseBody
	@RequestMapping("deleteLicenseImage.pr")
	public int deleteLicenseImage(int atNo,HttpSession session) {
		
		Image image = service.selectLicenseImage(atNo);
		
		int result = service.deleteLicenseImage(atNo);
		
		if(result>0) {
			new File(session.getServletContext().getRealPath("/"+image.getChangeName())).delete();
		}
